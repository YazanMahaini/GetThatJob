...
| --- | --- |
| First use | Create the four source/application folders, the generic letter design, the profile and evidence indexes, operations notes, search log, tracker, packet guide, and a per-workspace setup marker. Ask for the email plugin and submission mode; save `stop-and-wait` by default and preserve existing files and choices. If automatic submission is chosen, ask whether the applicant will select each opportunity or explicitly authorize a bounded standing scope; a missing scope remains `Not configured`. Pause after folder creation for the applicant to add documents; wait for explicit continue or skip before checking CVs. Preserve the pending handoff across sessions. If multiple supplied CVs have different designs, ask which of that applicant's CVs sets the visual style. |
...
| Opportunity choice, review and submission | Search targets guide discovery but do not authorize applying to every match. In `stop-and-wait` mode, hand off the checked draft for the applicant to choose whether to click Apply or direct a specific submission. In `automatic-submit` mode, final submission also requires that the applicant selected that specific opportunity or explicitly saved a standing automatic opportunity scope that covers it. Record the choice or scope match in the packet and tracker notes. Without it, stop and present the opportunity. A signature or equivalent declaration requires separate authorization for that specific application in either mode. |
