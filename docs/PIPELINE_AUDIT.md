...
| --- | --- |
| First use | Create the four source/application folders, an optional letter-reference folder, the profile and evidence indexes, operations notes, search log, tracker, packet guide, and a per-workspace setup marker. Ask for the email plugin and submission mode; save `stop-and-wait` by default and preserve existing files and choices. If multiple supplied CVs have different designs, ask which of that applicant's CVs sets the visual style. |
