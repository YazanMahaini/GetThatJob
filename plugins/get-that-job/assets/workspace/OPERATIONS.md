...
Setup runs once for this workspace and writes `.getthatjob/setup.json`. On first use, pause after creating the folders and invite the applicant to add documents. Wait for an explicit continue or skip before SetupChecker inspects CVs. Run SetupChecker after the user adds or changes source documents, and use ProfileIntake to index verified information. Read `APPLICATION_PROFILE.md` before asking for recurring form answers. Missing inputs are questions to resolve, not permission to invent facts.
...
Ask during first setup whether the applicant prefers `automatic-submit`; keep `stop-and-wait` when they do not explicitly choose automatic submission. In `stop-and-wait` mode, fill and check the application, then stop before final Apply/Submit so the applicant can click it or instruct Codex to submit that specific application. In `automatic-submit` mode, a completed, checked application may go to ApplicationFinalize only when the opportunity is also selected under the rule below. Never infer signing permission from either mode. Keep the saved mode across future runs until the applicant explicitly changes it.

## Opportunity choice

- Standing automatic opportunity scope: `Not configured`

`TARGET_ROLES.md` guides searching and screening; it is not permission to apply to every matching job. Before a final Apply/Submit action, establish that the applicant chose this specific employer and role, supplied its posting and asked to apply, or explicitly authorized a standing automatic opportunity scope that covers it. A generic request to "find me jobs" permits reversible draft preparation, not final submission. Record the choice or exact standing-scope match and its source in the opportunity packet and tracker notes. When no choice covers the role, present the shortlist or checked draft and ask which opportunities the applicant wants to pursue, even if `automatic-submit` is saved. A standing scope must be separately stated by the applicant, recorded here with its date, roles, locations, compensation conditions, and exclusions, and may be narrowed or withdrawn at any time. Do not broaden it automatically when search targets change. A direct instruction for one role applies only to that role.
...
3. For a strong, eligible role, create `applications/<ID>/README.md`, a tailored CV and a cover letter, plus any requested extras. Use supported facts from all approved CVs and the chosen CV design reference recorded separately in `CV_INDEX.md`. Use the generic cover-letter design in `CoverLetters/Template/` by default, or the applicant's own approved reference selected in `CoverLetters/README.md`. Preserve originals. Follow the requested output format. Pair every new Word document with a same-stem PDF in the packet, and use the PDF for review and upload unless the employer requires another format. Check text, layout, metadata, and comments.
4. Fill an accessible portal draft using verified answers when the user's request authorizes preparation. Check what persisted. Leave unknown required answers for the user. After each checked draft or material draft update, use ProfileBuilder to add newly verified, reusable answers to `APPLICATION_PROFILE.md`, preserving source and scope. Follow the saved submission mode and opportunity choice: `stop-and-wait` means hand off the draft before Apply/Submit; `automatic-submit` permits ApplicationFinalize only for a specifically selected role or one inside the applicant's explicit standing scope. Never sign without explicit instruction for that application.
