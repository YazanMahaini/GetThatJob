...
Use when the user explicitly directs final submission of a specific prepared application. Read workspace `AGENTS.md`, `OPERATIONS.md`, and `APPLICATION_PROFILE.md`, its tracker row and packet, the live posting, and the current portal draft. Verify the employer and destination, requisition, eligibility, deadline, fees, duplicate status, answers, and final document versions. Ask only about an unresolved required fact or commitment that cannot be sourced.
