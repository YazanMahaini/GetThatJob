...
1. **Set up and check the workspace.** On first use in that workspace, it asks once for your email plugin and submission preference, then creates only missing folders and blank records. If you choose automatic submission, it also asks whether you want to select jobs individually or set an explicit rule for which jobs it may apply to. It **pauses so you can add your CVs and other documents**. When you say to continue, it reads the available sources and asks the missing job-search questions together: paid or unpaid, suitable job types, and whether to search in your current country, remotely, or abroad. You may also share your LinkedIn profile link as another source. If your CVs have different designs, it asks which of your own CVs should set the style. Existing workspaces keep their files and resume without a new first-use pause.
...
5. **Apply only to jobs you choose.** By default, Codex gives you each checked draft and stops so you can decide whether to click Apply yourself or tell it to submit that application. If you chose automatic submission, it can submit a complete application only when you selected that particular job or it fits an automatic application rule you explicitly saved. A required fact, signature, or other blocker still needs you.
...
### How GetThatJob chooses jobs to search

You can tell GetThatJob the kinds of work you want in plain language: roles or fields, locations and remote options, pay expectations, employment types, and anything to avoid. It reads **all your approved CVs** for experience and skills. If you share your LinkedIn profile link, or an approved CV includes it, GetThatJob can also use that profile when accessible. It checks that the profile is yours and asks you to sign in through LinkedIn if access requires it. The profile is optional.

Your CVs and accessible LinkedIn profile are the main sources GetThatJob uses to **suggest** suitable roles. It also includes **every role or field you explicitly ask it to search**, even if your documents do not show that job title. You can confirm or correct the suggested priorities; a previous job on a CV or LinkedIn does not mean you want the same job again. If you explicitly ask it to choose targets from your sources, it can start a provisional search and tell you what it inferred. The saved priorities live in your private `TARGET_ROLES.md`. **LinkedIn Jobs** is a source of vacancies; it is separate from reading your own LinkedIn profile.

After you add your documents, GetThatJob asks which opportunities to include: **paid only or unpaid too** (an available stipend counts as paid), **full-time, part-time, internships, or contracts**, and **your current country only, remote work from there, or other countries**. It also checks any minimum pay, relocation, travel, and sponsorship limits. It saves your answers in `TARGET_ROLES.md` and reuses them on later runs. You can change them whenever you want.
...
It also asks whether to **stop after filling and checking each application** or **submit complete applications automatically for jobs you choose**. The default is to stop and let you click Apply yourself. You can instead tell Codex to submit a particular checked application. GetThatJob remembers this choice until you tell it to change it. Your preferred roles and locations guide the search; they do not mean you agree to apply to every match. If you choose automatic submission, GetThatJob asks whether you want to select jobs one at a time or give it an explicit rule covering the roles, locations, pay conditions, and exclusions you are comfortable applying to. Without that rule or a choice for a particular job, it stops and shows you the opportunity. Even with automatic submission, it will ask you before signing anything, paying a fee, or answering a question it cannot verify.
...
JobFinder searches LinkedIn Jobs and employer sites, verifies the live posting, checks duplicates and essential requirements, creates a tailored CV and cover letter, and fills a checked draft when accessible. ProfileBuilder then captures newly verified reusable answers. JobFinder shows the applicant the employer, role, fit, and any concerns so they can decide which opportunities to pursue. The saved submission mode controls who makes the final Apply action. Automatic submission additionally requires the applicant's choice for that specific opportunity or a separately authorized standing rule that covers it. Application signing always requires explicit authorization for that specific application. EmailConfirmationChecker verifies a matching receipt, and ApplicationFollowUp records later stages when asked.
